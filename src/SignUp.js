...
class SignUp extends Component {
  ...
  handleSignUp = (event) => {
    ...
    firebase.auth().createUserWithEmailAndPassword(email, password).then((userSignInOperation) => {

      this.createUserInDatabase(userSignInOperation.user).then(() => this.setState({
        email: '',
        password: '',
        redirectUpload: true
      }))
  ...
  handleSignIn = (event) => {
    event.preventDefault()
    const { email, password } = this.state
    firebase.auth().signInWithEmailAndPassword(email, password).then((userSignInOperation) => {
      this.setState({
        email: '',
        password: '',
      })
